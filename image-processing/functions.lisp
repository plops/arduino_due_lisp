...
(defun .abs2 (a)
  ...
  (let* ((b (make-array (array-dimensions a) :element-type 'single-float))
    ...
    (declare (type (array (complex single-float) 2) a)
	     ...
	     (type (array (complex single-float) 1) a1)
	     (type (simple-array single-float 1) b1))
    (dotimes (i n)
...
(defun .accum (dst b)
  ...
  (declare (type (array single-float 2) b)
	   (type (simple-array single-float 2) dst)
	   (values (simple-array single-float 2) &optional))
  (let* ((b1 (.linear b))
	 (dst1 (.linear dst))
	 (n (array-total-size dst)))
    (declare (type (simple-array single-float 1) b1 dst1))
    (dotimes (i n)
...
(defun extract (a &key
  ...
  (let* ((b1 (make-array (* h w) :element-type '(complex single-float)
    ...
    (declare (type (simple-array (complex single-float) 1) b1)
...
(defun .* (a b)
  (let* ((c (make-array (array-dimensions a)
    ...
    (typecase b
      ((array single-float *)
       (let ((b1 (.linear b)))
	 (declare (type (array single-float 1) b1 a1 c1))
	 (dotimes (i (length c1))
